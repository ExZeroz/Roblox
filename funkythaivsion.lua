...
local Window = Library:CreateWindow({Title = 'Reso Hub - Thai Version - Premieum Edition',Center = true, AutoShow = true,})
local Groups = {}
...
local column = Tabs['UI Settings']:AddRightTabbox() do
    ...
    section:AddInput('MyTextbox', {Default = 'ใส่ชื่อลงไป',Numeric = false,Finished = false,Text = 'ผู้เล่น',Placeholder = 'รอชื่อผู้เล่น',})
        local age = section:AddLabel('อายุ : ')
        local alive = section:AddLabel('ชีวิต : ')
        local id = section:AddLabel('ไอดี : ')
        local dis = section:AddLabel('ชื่อเล่น : ')
        local name = section:AddLabel('ชื่อ : ')
        ...
        Options.MyTextbox:OnChanged(function(val)
            if game.Players:FindFirstChild(val) then
                ...
                while game.Players:FindFirstChild(Options.MyTextbox.Value) do
                    wait(0.1)
                    age:SetText("อายุ : "..plr.AccountAge.." days")
                    alive:SetText('ชีวิต : '..(IsAlive(plr) and "ยังอยู่" or "ตาย"))
                    id:SetText('ไอดี : '..plr.UserId)
                    dis:SetText('ชื่อเล่น : '..plr.DisplayName)
                    name:SetText('Name : '..plr.Name)
                end
                age:SetText('อายุ : ')
                alive:SetText('ชีวิต : ')
                id:SetText('ไอดี : ')
                dis:SetText('ชื่อเล่น : ')
                name:SetText('ชื่อ : ')
            end
        end)
    end
...
local columncredit = Tabs['UI Settings']:AddRightTabbox() do
    local credit = columncredit:AddTab('--เครดิต--') do
            credit:AddLabel('<b><font color=\"rgb(255, 147, 0)\">SKX</font></b>'..' - คนเขียนสคริป')
            credit:AddLabel('<b><font color=\"rgb(1, 220, 255)\">Inori</font></b>'..' - เจ้าของ UI')
